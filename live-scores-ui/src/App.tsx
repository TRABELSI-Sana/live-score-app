...
export default function App() {
    ...
    const renderMatchCard = (match: (typeof allMatches)[number], variant: string) => {
        ...
        const suppressGoals = parsedScore?.total === 0;
        const recentEvents = sortedEvents.filter(
            (event) =>
                !isSubstitutionEvent(event.event) &&
                !isYellowCardEvent(event.event) &&
                (!suppressGoals || !isGoalEvent(event.event))
        );
        const goalEvents = suppressGoals ? [] : sortedEvents.filter((event) => isGoalEvent(event.event));
